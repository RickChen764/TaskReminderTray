...
- 详情面板默认展示本周每日工作安排，可切换上一周、本周和下一周；长标题自动滚动且采用双缓冲避免闪烁。
- 按开发、待跟进、等待输入整理工作，并识别任务/Bug、状态和 S/A/B/C 优先级。
- 状态变化使用持久通知，只有点击“已知晓”或关闭按钮才会清除；重启后仍会提醒。
- 工单标题仅用于展示；每行常驻“打开工单”和“复制单信息”按钮，复制内容为“编号 + 原始完整标题”。
- 每日存在多项安排时可按天展开全部工单。
- 可将任意工单设为当前重点，并设置 30 分钟、1 小时或次日等稍后提醒。
- 支持临期、当天到期和逾期提醒。
- 支持从右键菜单手动开启免打扰，并可配置多段每日重复时间段（含跨午夜）；免打扰期间保留待确认状态变化和稍后提醒，结束后恢复展示。
- 免打扰期间每 1 小时刷新一次，退出时立即刷新；账号密码登录会在内存中复用 Token，失效后自动重新登录。
